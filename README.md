...
```
Поскольку у нас есть "висящий" процесс, держащий "горизонт событий" транзаций с самого начала работы 10 обновляющих процессов, постгрес не сможет воспользоваться ни автовакуумом, ни hot-самоочисткой, поэтому таблица t1 при обновлениях в любом случае будет разбухать. Как правило, чтобы это исправить, необходимо выявить висящую транзакцию и завершить ее исполнение. В таком случае можно добиться постоянного tps во времени всего лишь с помощью настройки fillfactor (для hot-очистки и hot-обновлений) и autovacuum. 

Перед проведением нагрузочного тестирования необходимо более агрессивно настроить fillfactor и autovacuum.
```sql
ALTER TABLE t1 SET ( 
	   fillfactor = 70,
...
Чтобы определить "висящий" процесс, необходимо заглянуть в представление системного каталога pg_stat_activity. 
```sql
SELECT pid, query FROM pg_stat_activity 
WHERE state <> 'idle' AND backend_type = 'client backend';
  pid   |                           query                            
--------+------------------------------------------------------------
 157700 | update t1 set name = name where id = $1;
 157699 | select txid_current(); select pg_sleep(3600);
 157701 | savepoint v7;
 157702 | savepoint v82;
 157703 | savepoint v52;
 157704 | update t1 set name = name where id = $1;
 157705 | update t1 set name = name where id = $1;
 157706 | update t1 set name = name where id = $1;
 157707 | update t1 set name = name where id = $1;
 157708 | savepoint v2;
 157709 | update t1 set name = name where id = $1;
 158092 | SELECT pid, query FROM pg_stat_activity                   +
        | WHERE state <> 'idle' AND backend_type = 'client backend';
```

Затем необходимо завершить его исполнение, чтобы он не держал горизонт очистки.
```sql
SELECT pg_terminate_backend(157699);
...
Однако данная задача не подразумевает завершения висячего процесса, поэтому таблица t1 в любом случае будет разбухать. Таким образом, необходимо сделать так, чтобы работа поиска таплов по индексу не стагнировала с увеличением количества неактуальных таплов. Для этого нам необходимо искать значения не только по значению id, но еще и по какому-то значению, которое определяет версию той или иной строки. 
