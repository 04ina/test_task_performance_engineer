...
```
Поскольку у нас есть "висящий" процесс (транзакция), держащий "горизонт событий" транзаций с самого начала работы 10 обновляющих процессов, постгрес не сможет воспользоваться ни автовакуумом, ни hot-самоочисткой, поэтому таблица t1 при обновлениях в любом случае будет разбухать. Как правило, чтобы это исправить, необходимо выявить висящую транзакцию и завершить ее исполнение. В таком случае можно добиться постоянного tps во времени всего лишь с помощью настройки fillfactor (для hot-очистки и hot-обновлений) и autovacuum. 
