...
Устанавливать мы постгрес будем путем компиляции из исходников. Это позволит нам воспользоваться отладочными инструментами, если мы встретим "необъяснимое" поведение СУБД при построении планов. При компиляции с ключем -O0 большинство оптимизаций производительности при компиляции отключены, что может замедлить исполнение запроса на некое константное время, но не на порядок; поэтому мы все равно уложимся во временные диапазоны скорости исполнения запросов, указанные в задании. 
