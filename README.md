...
Чтобы найти и вывести нужное нам значение name, постгресу необходимо с помощью полного перебора посредством seq scan найти все строки, id которых равно значению 50000. Поскольку существует всего одна строка с id=50000 (из-за generate_series), использование полного перебора неоправдано.
```sql
...
```
###### Почему запрос медленный
Для каждой строки из таблицы t2 выполняется подзапрос, представляющий собой последовательное сканирование с фильтрацией по предикату t2.t_id = id. Затем уже результат выполнения данного подзапроса еще раз фильтруется по вышестоящему предикату. Данная ситуация осложняется тем, что план из-за фильтрации t2.t_id = t1.id не может быть материализован, поскольку при каждой итерации "внешнего цикла" внутренний запрос будет иметь разный результат.  
...
```
Также мы можем решить данную задачу несколько иначе. Для этого можно даже не переписывать запрос -- нужно лишь навесить индекс на столбец t1.id. В таком случае мы сможем быстро вычислять внутренний запрос.  
```sql
CREATE INDEX ON t1(id);
CREATE INDEX
