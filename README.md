...
```
Поскольку у нас есть висящий процесс, держащий "горизонт событий" транзаций с самого начала работы 10 обновляющих процессов, постгрес не сможет воспользоваться ни автовакуумом, ни hot-самоочисткой, поэтому таблица t1 при обновлениях в любом случае будет разбухаться. Как правило, чтобы это исправить, необходимо выявить висящую транзакцию и завершить ее исполнение. В таком случае можно добиться постоянного tps во времени всего лишь с помощью настройки fillfactor (для hot-очистки и hot-обновлений) и autovacuum. Однако данная задача не подразумевает завершения висячего процесса, поэтому таблица t1 в любом случае будет разбухать. Таким образом, необходимо сделать так, чтобы работа поиска таплов по индексу не стагнировала с увеличением количества неактуальных таплов. Для этого нам необходимо искать значения не только по значению id, но еще и по какому-то значению, которое определяет версию той или иной строки. 
