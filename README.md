...
```
###### Решение
Семантика запроса подразумевает, что мы должны должны вывести все строки из таблицы t2, значения t2.t_id которых не равны значениям t1.id ни одной строки таблицы t1. Этот запрос можно значительно улучшить, если воспользоваться конструкцией anti-join с использованием NOT EXISTS. 
```sql
...
```
Также мы можем воспользоваться иной конструкцией запроса, которая будет эквивалентна вышеописанному варианту благодаря безусловному улучшению дерева разбора запроса во время оптимизации.
```sql
